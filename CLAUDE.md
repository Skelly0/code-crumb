...
**Where it shows.** The main face's status line leads with it (`Opus is thinking`) and demotes the editor to a dim tag on the indicators row; a **child** orbital's row 5 shows it in place of the branch/cwd, which for a child is always its parent's and therefore redundant (a **top-level** orbital keeps the branch — a parallel window may be somewhere else); and the session list's info row gains a segment. Editors that cannot report a model are unchanged in every one of those places.
