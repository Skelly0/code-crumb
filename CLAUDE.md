...
test.js          Test runner — isolates HOME, loads 16 test files from tests/ (2039 tests); --quiet, name filters
...
- `~/.code-crumb-state` — the global state file. **The unified renderer no longer reads it**: the main face follows the chosen session's own file (see Main Session Policy). It is still written by every top-level hook and is what tmux mode (`--tmux`, via `readState()` with no argument) and external readers consume.
- `~/.code-crumb-sessions/{session_id}.json` — per-session state, for **every** session: the orbital subagents, the parallel editor windows, and the one the main face is currently following. Sticky fields set once and preserved across later writes: `taskDescription`, `parentSession`, `agentType`, `editor` (see Editor Provenance), `lastPromptAt` (see Main Session Policy) and the team fields. A Claude Code subagent's file is named `{parentSession}-agent-{agentId}.json` (see Subagent Attribution).
  ...
  - A non-agent `Stop` writes `idle` / `between turns` with `turnEnded: true` and **no** `stopped` (the session is between turns, not over). A late `PostToolUse`/`PostToolUseFailure` preserves it; the next turn's `PreToolUse`/`UserPromptSubmit` drops it. So the post-Stop contract on a session file is `stopped || turnEnded`: the global-owner guard re-stamps `stopped` for the owner's late `PostToolUse`, while a parallel window's late `PostToolUse` carries `turnEnded` instead — the renderer folds both.
...
- **Candidates** are the *live top-level* sessions: no `parentSession`, not a teammate, not `stopped`, not `isStale()`. A subagent or teammate is never chosen on its own.
...
- **Attention wins otherwise**: the highest `lastPromptAt` (`attentionAt`). That field is stamped by `SessionStart` (not from `/compact`) and `UserPromptSubmit` and is sticky, so it means "when the user last addressed this session".
...
`applyMainPolicy()` runs at the top of `checkState()` and returns early while a swap is animating. The **first** pick is silent (`adoptMain` directly); every later change animates through `SwapTransition`, whose `swap` frame calls `_executeSwap` → `adoptMain`. `adoptMain(id)` is the single place `mainSessionId` changes: it resets every per-session tracker together (`lastAppliedState`, `lastAppliedTimestamp`, `lastStopped`, `editorDead`, `lastEditorPid`, `candidatePid`, `lastNewWriteAt`, `lastMtime`, `lastForceReadTime`) and adopts the new session's `modelName`/`editor`/`cwd`/`gitBranch`, so a held work state or an armed PID can never leak onto the incoming face. `checkState()` is guarded **whole** while a transition is active — the guard used to sit inside the mtime branch, so on the ~3 ticks in 4 where the file was unchanged the rescue block, the fresh-read block and `idleCascade` all still acted on the session that was leaving.
...
**Where `lastPromptAt` comes from.** `update-state.js` stamps it on `UserPromptSubmit` and on a `SessionStart` whose `source` is not `compact`. Adapters (`base-adapter.js`) have no memory between events — each event is its own process — so the session file is the memory: the stamp is set on the first event of a session, on the first non-turn-ending event after a file marked `stopped` (a new turn), or when a live previous file has no stamp at all (self-healing across an upgrade or an `error`-first session); otherwise the previous stamp is carried forward. Adapters keep writing `stopped` at a turn end and never write `turnEnded`. `codex-wrapper.js` is long-lived, so it stamps on `thread.started` and `turn.started` and attaches the stamp to every write of that turn. `codex-notify.js` never stamps — its only event *is* a turn end.
...
4. the **info row**: `N tools · M files · age` for a top-level session, `<agentType | teamName | agent> · age ↳ parent` for a child. `formatAge(ms)` renders `3s` / `2m` / `1h`.
...
- **`applyMainPolicy()` runs above `checkState`'s `try`.** A throw inside it — the pure `pickMainSession` or `orbital.setMainSession` — would escape the interval callback and kill the render loop, where everything below it is caught. Both are simple and neither has thrown, but the asymmetry is deliberate only in the sense that nobody moved it.
