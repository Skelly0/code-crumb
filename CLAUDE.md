...
setup.js         Multi-editor setup — installs hooks (setup.js [claude|codex|codex-notify|opencode|openclaw|uninstall] [--autolaunch]; opencode also takes --install/--uninstall). `uninstall` removes both the Claude Code hooks and the Codex hooks file. setupClaude/uninstallClaude, setupCodex/uninstallCodex/buildCodexHooks, setupOpenCode/uninstallOpenCode, buildFaceHooks and printClaudeUsage are importable (the demo hint is printed only when demo.js exists; the npm tarball omits it)
test.js          Test runner — isolates HOME, loads 16 test files from tests/ (2252 tests on win32); --quiet, name filters
demo.js          Demo script — cycles through all face states in single-face mode; writes a `demo-main` session file (with `lastPromptAt`) so the renderer's main-session policy picks it, and unlinks it on exit and on SIGINT, SIGTERM and SIGHUP
...
- `~/.code-crumb-stats.json` — persistent stats (streaks, records, session counters), plus `sessionCounters`, a per-session-id map `{toolCalls, filesEdited, start, commitCount, creditedMs, lastSeen, counted}` (24h TTL, 50 entries, 200 files each). Session and global files report the writer's own counters; `stats.session` mirrors its owner's entry, switching ownership restores counters rather than zeroing them, `daily.sessionCount` goes up once per session id, and `creditedMs` stops the same time being added to `cumulativeMs` twice.
...
The suite is **2252 tests across 16 files** (on win32): test-shared 42, test-state-machine 566, test-themes 72, test-animations 42, test-particles 49, test-face 282, test-grid 345, test-accessories 24, test-teams 39, test-launch 45, test-adapters 260, test-transition 31, test-emotions 176, test-platform 108, test-subagents 74, test-attention 97. Per-file counts shift across platforms because some tests are platform-conditional — test-adapters gains 7 POSIX-only signal tests (wrapper, engmux and demo SIGTERM/SIGHUP) and test-platform has one win32-only real `shell:true` quoting round-trip, so treat the total as the figure to check. One test, `test-shared.js › getIsWorktree`, fails inside a git worktree by construction — `.git` is a file there — which is environmental, not a regression. **`test-adapters.js` dominates wall time** — it spawns real `update-state.js` and adapter subprocesses, so a full run is mostly waiting on node cold starts; use `node test.js <filter>` while iterating on anything else. Treat the count as tests, not as coverage: a round of behavioural conversion collapsed many one-line source greps into fewer, multi-assertion tests, which lowers the number while raising the assertions behind it.
...
The **second review pass (Sep 2026)** added regression tests across the suite, every one checked to fail against the pre-fix sources:
...
- **test-platform**: `quoteArg` cmd metacharacters (plus a win32 `shell:true` round-trip); `%`/`&` in `buildRendererCommands`; the staged stale-lock race.
...
- **test-emotions**: `needsRescue`/`RESCUE_EXCLUDE`; `policySessions` dead-session projection; `splitKeys`; the `tmuxDisplayState` table; source checks for the startup record, new-pid reset, resize guard, subagent count and stdin split.
...
- **test-attention**: `/compact` keeping counters; alternating windows; `StopFailure`; payloads over 1 MB; non-string fields.
...
- **A lifecycle event from a parallel window still takes over `stats.session`** (SessionStart/SubagentStart/PreCompact…) and wipes the owner's `activeSubagents`. Per-session counters survive it now; the agent bookkeeping does not.
...
- **Renderer startup still has a sibling of the boot-replay bug.** A *non-stopped* write 15–120s old is skipped during the first 5s and then applied anyway afterwards.
- **`wt` splits on `;` even inside quotes**, so a renderer path containing `;` breaks the Windows Terminal launch (no injection — `wt` only treats it as its own subcommand separator). Fixing it means escaping `\;` in `launch.js`.
...
- **`quoteArg` (win32 `shell: true`) cannot neutralise `!VAR!`** if delayed expansion is enabled in the registry (off by default for `cmd /c`). Every other cmd metacharacter is quoted, an embedded `"` becomes `""`, `%` is emitted as `"^%"`, and line breaks become spaces.
