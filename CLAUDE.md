...
- **`STICKY_FIELDS` contains a dead entry.** `update-state.js`'s module-scope `STICKY_FIELDS` lists `'editor'`, but the loop's `!extra[field]` guard can never be true for it — `extra.editor` is always set. Provenance is actually preserved by the global owner guard. Harmless, but misleading to read.
