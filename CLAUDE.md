...
The suite is **2350 tests across 16 files** (on win32; 2362 on Linux/macOS): test-shared 42, test-state-machine 589, test-themes 72, test-animations 42, test-particles 49, test-face 296, test-grid 353, test-accessories 24, test-teams 39, test-launch 45, test-adapters 278, test-transition 31, test-emotions 184, test-platform 128, test-subagents 74, test-attention 104. Per-file counts shift across platforms because some tests are platform-conditional — test-adapters gains 10 POSIX-only tests (wrapper, engmux and demo SIGTERM/SIGHUP, the wrapper's late-event retirement, and the OpenCode plugin's per-session ordering and turn-end drain, which need a shell-script stand-in for node), and test-platform gains 3 POSIX-only tests (backup file modes, the `code-crumb.sh` symlink) and has one win32-only real `shell:true` quoting round-trip, so treat the total as the figure to check. One test, `test-shared.js › getIsWorktree`, fails inside a git worktree by construction — `.git` is a file there — which is environmental, not a regression. **`test-adapters.js` dominates wall time** — it spawns real `update-state.js` and adapter subprocesses, so a full run is mostly waiting on node cold starts; use `node tests/run.js <filter>` while iterating on anything else. Treat the count as tests, not as coverage: a round of behavioural conversion collapsed many one-line source greps into fewer, multi-assertion tests, which lowers the number while raising the assertions behind it.

Where things are tested:

| File | Covers |
|---|---|
| test-shared | `safeFilename` edge cases |
...
| test-grid | `MiniFace` and `OrbitalSystem`: orbit geometry and the layout-invariant sweep, grouping/tethers/labels, `isOwnedByLiveProcess` and the purge rules, session loading, orbital row 5 |
...
| test-platform | quoting (`quoteArg`, `shQuote`, `wt`), `writeJsonAtomic`, the spawn and stats locks, setup.js against temp config files, launch.js exit codes, `code-crumb.sh` through a symlink |
...
- **A regression test must fail first.** Every test added by the review passes was checked to fail against the pre-fix sources, and they sit in labelled blocks (`Third review pass`, `review fixes`). Keep doing both. What each pass fixed is written up in `CHANGELOG.md`.
