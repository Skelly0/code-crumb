...
const PALETTES = [
  ...
  // 4: Sunset/ (warm oranges/reds/golds/purples)
  {
    ...
    timelineColors: {
      ...
      subagent:    [130,60,150],  responding:[50,120,110],
      starting:  [105,70,140],
      spawning:    [120,85,158],
...
// A face state read from a file, or the fallback. Every lookup by state is
// `themes[state] || themes.idle`, and an inherited name (`__proto__`,
// `constructor`, `toString`) got past the `||` as an object or a function:
// the face and the ring drew blank and the title read "is undefined".
const STATE_NAMES = new Set(Object.keys(themes));
function knownState(s, fallback = 'idle') {
  return typeof s === 'string' && STATE_NAMES.has(s) ? s : fallback;
}
